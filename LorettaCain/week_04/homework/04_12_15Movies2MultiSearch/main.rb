...
get '/movie' do
  ...
  @movie = params[:title].upcase

  @movie_url = "http://omdbapi.com/?s=#{ @movie }"

  @movie_info = HTTParty.get @movie_url;

  @search_title_info = @movie_info['Search'].collect {|movies| movies['Title']}

  
#USE THESE THREE BELOW TO SEARCH YOUR DATABASE:

  # @search_imbdID_info = @movie_info['Search'].collect {|movies| movies['imdbID']}

  # @search_Poster = @movie_info['Search'].collect {|movies| movies['Poster']}


  # @zipInfo = @search_title_info.zip(@search_imbdID_info, @search_Poster)
...
# for Multipsearch always go to omdb because there might be a new movie out. The first time you click one of them, go get it from OMDB and put it in my database, the second time you click it, it should come from my database.
...
get '/movie/:imdbID' do
  ...
  @movie = Movie.where :url => params[:imdbID]
  ...
  if @movie != nil
    ...
    @movie.url = "http://omdbapi.com/?i=#{ params[:imdbID] }"
